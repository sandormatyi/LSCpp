...
Fractal::Fractal(coord_t xCenter, coord_t yCenter, coord_t zoom, fractal_value_t maxN) :
        _xCenter(xCenter),
        _yCenter(yCenter),
        _maxN(maxN),
        _zoom(zoom),
...
fractal_value_t Fractal::getFractalValue(int screenX, int screenWidth, int screenY, int screenHeight) const
{
...
fractal_value_t Fractal::changeMaxN(fractal_value_t delta)
{
    fractal_value_t oldMaxN = _maxN;
    _maxN = std::fmaxf(_maxN + delta, 1.0);
    return _maxN - oldMaxN;
}

fractal_value_t Fractal::getMaxN() const
{
    return _maxN;
}
...
void Fractal::setMaxN(fractal_value_t maxN)
{
    _maxN = maxN;
}
